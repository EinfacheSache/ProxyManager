...
public final class KeyedCommandListener implements CommandHandler<KeyedPlayerCommandPacket> {

    private final VelocityServer proxyServer;

    @Inject
    public KeyedCommandListener(final VelocityProxyManager plugin) {
    ...
    public void onCommand(final PacketReceiveEvent event) {
        if (!(event.getPacket() instanceof KeyedPlayerCommandPacket packet)) {
            return;
        }
        ...
        queueCommandResult(
                ...
                new CommandExecuteEvent.InvocationInfo(null, null)
        );
    }
